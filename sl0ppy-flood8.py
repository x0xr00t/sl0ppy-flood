...
# HTTP Methods for 405 bypass and 308 bypass
BYPASS_METHODS = ['OPTIONS', 'HEAD']  # Advanced methods for 308 bypass
...
class Spammer(Thread):
    ...
    def request(self):
        ...
        try:
            ...
            # Advanced 308 Bypass to get 200 OK
            if req.status_code == 308 and 'Location' in req.headers:
                redirect_url = req.headers['Location']
                print(f"[308 BYPASS] Redirecting to: {redirect_url}")
                # Follow the redirect path until 200 OK
                while True:
                    follow = requests.get(redirect_url, headers=self.headers, allow_redirects=False)
                    print(f"[FOLLOW] {redirect_url} | Status: {follow.status_code}")
                    if follow.status_code == 200:
                        print(f"[SUCCESS] 200 OK at {redirect_url}")
                        break
                    elif follow.status_code == 308 and 'Location' in follow.headers:
                        redirect_url = follow.headers['Location']
                    else:
                        print(f"[FAIL] Unexpected status: {follow.status_code}")
                        break
